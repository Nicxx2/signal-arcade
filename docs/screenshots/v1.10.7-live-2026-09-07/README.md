# Signal Arcade v1.10.7 — live screenshots

Captured on **7 September 2026** from the locally deployed v1.10.7 release image:
`sha256:c16e661edbee6c64a5865b5d21f030cf52b871322c9962f72ab8502e02139f34`. Earlier screenshot folders remain unchanged.

The 18 PNGs show actual live HTTP/WebSocket data in a fresh headless Microsoft Edge profile,
with `en-GB` formatting, Europe/London time and Low 3D graphics. Desktop viewports are 1440 pixels
wide; mobile viewports use 390×1000. Individual cards use their rendered size. The manifest records
exact dimensions, timestamps, hashes and captions.

No synthetic responses, altered figures, hidden warnings or selected performance winners are used.
The live battle was Sizing; the recorded comparison was the latest available recap.
Results uses the default comparison group. Replay inspection selects an early checkpoint by keyboard,
rather than choosing a peak. Viewport captures may end partway through longer pages.

The capture blocks HTTP writes and opens no credential editor. It does not alter engine, learning,
provider, risk or permission settings. Model identities, qualification, support and all numbers are
time-specific observations, not promises of profitability or future qualification.

The run received 240 WebSocket frames and recorded no page exceptions, failed
non-authentication HTTP responses or attempted mutations.

All images were visually inspected. After the images and manifest were saved, the browser exited
but its Node capture helper stalled during shutdown. Only that verified helper process was closed;
no screenshot was changed. The capture script now bounds this shutdown wait for future runs.
...
| [08-replay-receipts.png](08-replay-receipts.png) | Latest paper fill receipts with recorded fees, impact and latency. |
...
| [11-champion-arena.png](11-champion-arena.png) | A live Sizing comparison: measured advantage and uncertainty, not win probability. |
