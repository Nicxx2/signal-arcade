...
ENTRY_VALIDATION_VERSION = "entry-top-group-v2"
...
VETO_SCORING_VERSION = "baseline-fallback-v1"
...
def _policy_identity_key(item: LearningObservation | LearningEvidenceEpisode) -> str:
    return _policy_identity_digest(
        LEARNING_EVIDENCE_SCHEMA_VERSION,
        item.mint,
...
def _policy_identity_digest(
    ...
    # Only immutable identity material is reused. Eligibility, the retained first-episode
    # receipt, clocks and outcomes must still be checked against current evidence every time.
    # Bound the process-local cache across long runs, seasons and configuration changes.
    return _stable_digest(
        (evidence_schema, mint, risk_mode, configuration, baseline, feature_schema)
    )


def _policy_evidence_cohort(episodes: Sequence[LearningEvidenceEpisode]) -> str:
    """Fingerprint the immutable Policy population, including unavailable resolved labels."""
    key = str(PRIMARY_HORIZON_SECONDS)
    return "v1:" + _stable_digest(
        [
            {
                "id": item.episode_id,
                "contract": _policy_identity_key(item),
                "entry_at": item.entry_at.astimezone(UTC).isoformat(),
                "features": item.features,
                "outcome": item.checkpoints[key].net_return,
                "observed_at": item.checkpoints[key].observed_at.isoformat(),
                "missing_reason": item.checkpoints[key].missing_reason,
            }
            for item in sorted(episodes, key=lambda value: value.episode_id)
            if key in item.checkpoints
        ]
    )


def _policy_identity(episode: LearningEvidenceEpisode) -> tuple[str, str, str] | None:
    if not (
    ...
    return (
        _policy_identity_key(episode),
        episode.entry_at.astimezone(UTC).isoformat() if eligible else "",
        episode.episode_id if eligible else "",
    )


def _matches_policy_identity(episode: LearningEvidenceEpisode, identity: tuple[str, str]) -> bool:
    """A reused mint-season ID cannot turn a later entry into original forward proof."""
    if identity[1] != episode.episode_id:
        return False
    try:
        first_entry = datetime.fromisoformat(identity[0])
    except ValueError:
        return False
    return (
        first_entry.utcoffset() is not None
        and episode.entry_at.utcoffset() is not None
        and first_entry == episode.entry_at
    )
...
class LearningEngine:
    ...
    def __init__(
        ...
        self.collection_diagnostics = CollectionDiagnostics()
        ...
        database.remember_policy_identities(
            [
                identity
                for item in self.evidence_episodes.values()
                if (identity := _policy_identity(item)) is not None
            ]
        )
        self._policy_identities = database.policy_identities(
            {_policy_identity_key(item) for item in self.observations.values()}
            | {_policy_identity_key(item) for item in self.evidence_episodes.values()}
        )
        ...
        self._pending_authority_states: dict[tuple[str, ChallengerSkill], ChallengerSkillState] = {}
    ...
    def has_due_market_work(self, mint: str, now: datetime) -> bool:
        """Check one mint under the owner's lock before dispatching checkpoint work.

        Keep the Discovery and Policy clocks separate. Overdue work still runs to record
        unavailable outcomes; a pending record with every checkpoint still needs closure.
        This does not change event admission, route validation or checkpoint deadlines.
        """

        def due(entered: datetime, checkpoints: dict[str, LearningCheckpoint]) -> bool:
            age = max(0.0, (now - entered).total_seconds())
            return len(checkpoints) == len(LEARNING_HORIZONS_SECONDS) or any(
                str(horizon) not in checkpoints and age >= horizon
        ...
        if (
            ...
            and observation.status == LearningObservationStatus.PENDING
            and due(observation.created_at, observation.checkpoints)
        ):
            return True
        for episode_id in self._evidence_episode_ids_by_mint.get(mint, ()):
            ...
            if (
                episode.lane == LearningEvidenceLane.POLICY
                and episode.status == LearningEvidenceStatus.PENDING
                and due(episode.entry_at, episode.checkpoints)
            ):
                return True
        return False
    ...
    def prepare_next_training(self, runtime_context: tuple[Any, ...] = ()) -> TrainingJob | None:
        ...
        try:
            ...
            workspace._policy_identities = dict(self._policy_identities)
            workspace.models = []
    ...
    def set_mode(self, mode: LearningMode, *, preserve_participation: bool = False) -> None:
        ...
        if mode == LearningMode.ACTIVE:
            ...
            if skill_candidate is not None:
                self._activate_skill(skill_candidate, grant_consent=True)
                return
        ...
        *,
        ...
        if (
            ...
            if (
                ...
                and reserved[1] == episode.episode_id
                and not _matches_policy_identity(episode, reserved)
            ):
                # Preserve the later journal entry, but a pruned original owns this identity.
                episode.qualification_eligible = False
    ...
    def observe_market(
        ...
        *,
        ...
        for horizon in LEARNING_HORIZONS_SECONDS:
            ...
            changed += 1
            self.collection_diagnostics.record(
                "discovery",
                horizon,
                "expired"
                if age > horizon + CHECKPOINT_GRACE_SECONDS
                else "usable"
                if observation.checkpoints[key].net_return is not None
                else "unavailable",
                trajectory_id=observation.observation_id,
            )
        ...
        for episode_id in self._evidence_episode_ids_by_mint.get(state.mint, ()):
            ...
            for horizon in LEARNING_HORIZONS_SECONDS:
                ...
                self.collection_diagnostics.record(
                    ...
                    if episode.checkpoints[key].net_return is not None
                    else "unavailable",
    ...
    def due_checkpoint_mints(
        ...
        *,
        ...
        routes. The final 15 seconds get priority within each lane; otherwise failed attempts
        rotate behind unserved mints. Neither urgency nor a retry extends the original clock.
        """
        ...
        items = [
            ...
            ("policy", item.mint, item.entry_at, item.checkpoints, getattr(item, "episode_id", ""))
            for item in self.evidence_episodes.values()
            ...
            for horizon in LEARNING_HORIZONS_SECONDS:
                ...
                if str(horizon) not in checkpoints and due_at <= now <= due_at + timedelta(
                    ...
                    if diagnostics is not None:
                        ...
                        diagnostics.record(lane, horizon, channel + "_due")
                    if selection is not None or diagnostics is not None:
                        targets.setdefault(mint, []).append((lane, horizon))
        ...
        for mint, policy_due in lanes["policy"].items():
            ...
            if discovery_due is not None:
                # One Policy-lane fetch serves both clocks; retain the earlier eligible one.
                lanes["policy"][mint] = min(policy_due, discovery_due)
        ...
        urgent_before = now - timedelta(
            seconds=CHECKPOINT_GRACE_SECONDS - CHECKPOINT_URGENT_SECONDS
        )
        queues = {
            lane: sorted(
                mints,
                key=lambda mint: (
                    mints[mint] > urgent_before,
                    served.get(mint, -1),
                    mints[mint],
                    mint,
                ),
        ...
        if selection is not None:
            ...
            if isinstance(selection, CollectionTargets):
                selection.identities.update({mint: targets.identities[mint] for mint in selected})
        return selected
        ...
        *,
        ...
        for observation in self.observations.values():
            ...
            for horizon in LEARNING_HORIZONS_SECONDS:
                ...
                changed += 1
                self.collection_diagnostics.record(
                    "discovery", horizon, "expired", trajectory_id=observation.observation_id
                )
        ...
        for episode in self.evidence_episodes.values():
            ...
            for horizon in LEARNING_HORIZONS_SECONDS:
                ...
                changed += 1
                self.collection_diagnostics.record(
                    "policy", horizon, "expired", trajectory_id=episode.episode_id
                )
                primary_changed = primary_changed or horizon == PRIMARY_HORIZON_SECONDS
                if horizon == PRIMARY_HORIZON_SECONDS:
                    primary_cohorts.add((episode.risk_mode, episode.configuration_fingerprint))
    ...
    def assess(
        ...
        *,
        ...
        applied = bool(
            ...
            and _entry_validation_current(self.active_model)
    ...
    def status(
        ...
        # The owner keeps observations fixed throughout this synchronous read. Reuse only
    ...
    def _complete_discovery_features(self, observation: LearningObservation) -> bool:
        ...
        if checks is None:
            return _observation_features_complete(observation)
        key = id(observation)
        if key not in checks:
            checks[key] = _observation_features_complete(observation)
        return checks[key]
    ...
    def _status(
        ...
        samples = self._training_rows(
            mode=training_mode,
            configuration_fingerprint=training_configuration,
        ...
        evidence_lanes = self.evidence_lane_status(
            training_summary=(training_mode, training_configuration, len(samples))
        )
        return {
            ...
            "champion_impact": (
                self.champion_impact(skill_statuses, impact_context, demo_mode=demo_mode)
    ...
    def evidence_lane_status(
        self, *, training_summary: tuple[RiskMode, str | None, int] | None = None
    ) -> list[dict[str, Any]]:
        """Summarize distinct evidence lanes without implying that all rows can qualify policy."""

        feature_check = (
            self._complete_discovery_features
            if getattr(self._status_policy_cache, "features", None) is not None
            else _observation_features_complete
        )
        mode = self.current_risk_mode
        configuration = self.configuration_fingerprint()
        baseline = self.baseline_version()
        # A dashboard already selected these exact Discovery training rows. Reuse only its
        # count, never Policy proof/coverage populations or a cache surviving this response.
        # Standalone callers and a changed cohort still perform their own fresh selection.
        training_count = (
            training_summary[2]
            if training_summary is not None and training_summary[:2] == (mode, configuration)
            else len(
                self._training_rows(
                    mode=mode,
    ...
    def champion_impact(
        ...
        context: tuple[str | None, str | None, int],
        *,
        demo_mode: bool,
    ) -> dict[str, Any]:
        """Spectator-only view, sharing the status pass's bounded Policy selection."""
        versions = dict(self.active_skill_versions)
        boundaries = []
        valid = self.mode == LearningMode.ACTIVE and not demo_mode and bool(versions)
        for skill in skill_statuses:
            version = versions.get(skill["skill"])
            if version is None:
                continue
            state = self._current_skill_state(ChallengerSkill(skill["skill"]))
            ...
            if (
                ...
                or skill["health"]["state"] == "suspended"
            ):
                valid = False
                continue
            boundaries.append(max(state.joined_at, artifact.created_at))
        valid = valid and len(boundaries) == len(versions)
        rows = (
        ...
        return build_impact(
            rows,
            versions=versions,
            since=max(boundaries) if valid else None,
            season_id=context[0],
            profile=context[1],
            normal_hold=context[2],
    ...
    def nonlinear_entry_status(self) -> dict[str, Any]:
        ...
        latest_fit = max(
            (artifact for artifact in (latest_linear, latest_nonlinear) if artifact is not None),
            key=lambda artifact: (artifact.created_at, artifact.version),
            default=None,
        )
        eligible_training_count = latest_fit.training_count if latest_fit is not None else 0
        status = "collecting"
        if latest_nonlinear is not None:
            if state is not None and state.suspended_version == latest_nonlinear.version:
                ...
                state is not None
                and state.active_version == latest_nonlinear.version
                and _entry_validation_current(latest_nonlinear)
            ):
                status = "active"
                ...
                state is not None
                and state.testing_version == latest_nonlinear.version
                and _entry_validation_current(latest_nonlinear)
            ):
                status = "testing"
            elif (
                state is not None
                and latest_nonlinear.version in state.pending_versions
                and _entry_validation_current(latest_nonlinear)
            ):
                status = "queued"
            elif (
        ...
        return {
            ...
            "training_count_at": latest_fit.created_at.isoformat() if latest_fit else None,
    ...
    def entry_outcome_availability(
        ...
        discovery_resolved = [
            ...
            and observation.feature_schema_version == FEATURE_SCHEMA_VERSION
            and observation.baseline_version == self.baseline_version()
            and feature_check(observation)
            and not self._observation_has_policy_twin(observation)
        ...
        for skill in (
            ...
            for version in versions:
                ...
                if artifact is None or artifact.skill != skill:
                    continue
                if artifact.schema_version == "challenger-skill-coach-v1" and not (
                    self._coach_evaluation_context(artifact, state, observation)
                ):
                    continue
    ...
    def _coach_evaluation_context(
        ...
        """Separate research/battle context from a fresh crown join and active health."""
        if state.champion_version != artifact.version:
            return observation.active_skill_versions == artifact.dependency_versions
        order = (
            ChallengerSkill.ENTRY,
            ChallengerSkill.MANIPULATION,
            ChallengerSkill.SIZING,
        ...
        downstream = {skill.value for skill in order[order.index(artifact.skill) + 1 :]}
        context = {
            name: version
            for name, version in observation.active_skill_versions.items()
            if name not in downstream
        }
        if self.active_skill_versions.get(artifact.skill.value) == artifact.version:
            return bool(
                self._valid_activation_proof(state, artifact, dependencies)
                and context == {**dependencies, artifact.skill.value: artifact.version}
                and state.joined_at is not None
                and observation.created_at > state.joined_at
            )
        if context != dependencies:
            return False
        proof = state.activation_proof
        if (
            proof.get("policy") != "coach-join-v2"
            ...
            state.activation_proof = {
                **({"recovery": proof["recovery"]} if "recovery" in proof else {}),
                ...
                "started_at": observation.created_at.isoformat(),
            }
            self.database.save_challenger_skill_state(state)
        return True
    ...
    def _publish_entry_artifact(
        ...
        *,
        ...
        artifact = ChallengerSkillArtifact(
            ...
            qualified=model.qualified and _entry_validation_current(model),
        ...
        *,
        ...
        if _skill_qualified(artifact) and not self._redundant_exit_candidate(state, artifact):
            ...
            state.pending_versions = [
                ...
                and version not in {state.testing_version, state.champion_version}
                and (
                    (self.skill_artifacts[version].schema_version == "challenger-skill-coach-v1")
    ...
    def _preferred_pending_version(self, state: ChallengerSkillState) -> str | None:
        ...
        # Scores from different populations are not comparable. Serve the oldest waiting
        # cohort; its eventual replacement still needs the existing common-forward battle.
        oldest = min(candidates, key=lambda item: (item.created_at, item.version))
        if not oldest.evidence_cohort_digest or oldest.training_cutoff_at is None:
        ...
        candidates = [
            ...
            and item.feature_names == oldest.feature_names
        ]
        ...
        if best_linear is not None and best_nonlinear is not None:
            linear_policy_cohort = best_linear.parameters.get("policy_evidence_cohort")
            if (
                not isinstance(linear_policy_cohort, str)
                or not linear_policy_cohort.startswith("v1:")
                or linear_policy_cohort != best_nonlinear.parameters.get("policy_evidence_cohort")
            ):
                # Legacy artifacts cannot reconstruct a historical Policy population from today's
                # journal. Preserve them and use waiting order, never manufacture provenance.
                return oldest.version
    ...
    def _start_next_skill_tournament(self, state: ChallengerSkillState) -> None:
        ...
        if state.champion_version is None:
            ...
            state.last_tournament = {
                ...
                "selection": "held_out_proof_with_same_cohort_complexity_preference",
            }
        ...
        state.last_tournament = {
            ...
            "common_usable_count": 0,
            **(
                {"action_scoring": VETO_SCORING_VERSION}
                if state.skill in {ChallengerSkill.ENTRY, ChallengerSkill.MANIPULATION}
                else {}
            ),
        }
    ...
    def _advance_entry_tournaments(self) -> None:
        ...
        for key, state in list(self.skill_states.items()):
            ...
            candidate_available = bool(
                ...
                and _entry_validation_current(candidate)
            ...
            for observation in usable:
                ...
                candidate_winner_vetoes += int(
                    _veto_applies(candidate_receipt) and outcome is not None and outcome > 0
                )
                champion_winner_vetoes += int(
                    _veto_applies(champion_receipt) and outcome is not None and outcome > 0
                )
    ...
    def _activate_skill(
        ...
        *,
        ...
        dependency_reason: str = "dependency_activated",
    ) -> None:
        ...
        # Prepare detached authority; publish only after the complete transaction commits.
        # Callers own the event boundary, with no await between commit and publication.
        ...
        self.database.save_challenger_authority(
            ...
            clear_legacy_model=True,
        )
        for item in [*changed_states, state]:
            key = (item.cohort_key, item.skill)
            previous = self.skill_states.get(key)
            if previous is None:
                self.skill_states[key] = item
            else:
                # Tournament callers retain this object. Publish committed fields into it
                # rather than leaving the caller holding an obsolete authority snapshot.
                for field in type(item).model_fields:
                    setattr(previous, field, getattr(item, field))
                previous.__pydantic_private__ = item.__pydantic_private__
    ...
    def _promote_active_skill(
        ...
        *,
        ...
        if (
            ...
            return
        self._activate_skill(artifact, dependency_reason="dependency_promoted")

    def _persist_revoked_authority(self) -> None:
        """Retry failed revocation persistence before any subsequent grant of authority."""
        if not self._pending_authority_states:
            return
        self.database.save_challenger_authority(
            list(self._pending_authority_states.values()),
            self.active_skill_versions,
            mode=self.mode.value,
            clear_legacy_model=True,
        )
        self._pending_authority_states.clear()
    ...
    def _deactivate_all_skills(self) -> None:
        for state in self.skill_states.values():
            ...
            state.active_dependencies = {}
            state.updated_at = datetime.now(UTC)
            self._pending_authority_states[(state.cohort_key, state.skill)] = state
        self.active_skill_versions = {}
        self.active_model = None
        if self.mode != LearningMode.OFF:
            self.mode = LearningMode.SHADOW
        if self._pending_authority_states:
            self._persist_revoked_authority()
        else:
            self.database.save_challenger_authority([], {}, mode=self.mode.value)
    ...
    def _skill_health(
        ...
        *,
        ...
        cohort = (artifact.risk_mode, artifact.configuration_fingerprint, artifact.baseline_version)
        # A governance pass may share its exact Policy population across skills. This is a
        # local scratch map, discarded before another checkpoint or authority change; receipts,
        # dependencies and health are always evaluated below, never cached.
        if policy_rows is None or cohort not in policy_rows:
            evidence = self._policy_evidence(
            ...
            if policy_rows is not None:
                policy_rows[cohort] = evidence
        else:
            evidence = policy_rows[cohort]
        for observation in evidence:
            ...
            if receipt is not None:
                ...
                resolved = all(str(horizon) in observation.checkpoints for horizon in horizons)
            if not resolved:
                continue
        ...
        for observation in observed:
            receipt = observation.challenger_evaluations.get(version)
            if receipt is None:
                # Missing expected receipts are unavailable evidence, not invisible samples.
                continue
            actual, baseline = (
    ...
    def _skill_join_evidence(
        ...
        *,
        ...
        # Coach crowns additionally need a durable fresh join context after their battle.
        downstream = (
        ...
        coach_context_valid = True
        if is_coach:
            state = self._current_skill_state(artifact.skill)
            proof = state.activation_proof if state is not None else {}
            coach_context_valid = bool(
                proof.get("policy") == "coach-join-v2"
                and proof.get("artifact_version") == artifact.version
                and proof.get("dependencies") == dependencies
            )
            try:
                started_at = datetime.fromisoformat(str(proof.get("started_at", "")))
                if started_at.tzinfo is None:
                    raise ValueError("join context must have a timezone")
                since = max(since, started_at)
            except (ValueError, TypeError):
                coach_context_valid = False
        if independent:
    ...
    def _govern_skill_ensemble(self) -> None:
        ...
        for skill_name, version in list(self.active_skill_versions.items()):
            try:
                ...
                continue
            health = self._skill_health(skill, version, policy_rows=policy_rows)
        ...
        for skill in order:
            ...
            if state and artifact and state.suspended_version == artifact.version:
                ...
                if recovery.get("ready") and (
                    ...
                    self._activate_skill(artifact, recovering=True)
                    return
                continue
            if (
                state is None
                ...
                continue
            if skill == ChallengerSkill.ENTRY and self._skill_activation_candidate() != artifact:
                continue
            if (
                skill != ChallengerSkill.ENTRY
                or artifact.schema_version == "challenger-skill-coach-v1"
    ...
    def _prune_complete_history(self) -> None:
        ...
        for episode_id in self.database.prune_learning_evidence(MAX_COMPLETED_OBSERVATIONS):
            ...
            removed = True
        if removed:
            retained_keys = {_policy_identity_key(item) for item in self.observations.values()} | {
                _policy_identity_key(item) for item in self.evidence_episodes.values()
            }
            self._policy_identities = {
                key: value for key, value in self._policy_identities.items() if key in retained_keys
            }
            self._invalidate_timing_validation()
...
def _veto_applies(receipt: ChallengerEvaluationReceipt) -> bool:
    """A proposal outside learned support cannot override the Baseline."""
    return (
        receipt.skill in {ChallengerSkill.ENTRY, ChallengerSkill.MANIPULATION}
        and receipt.in_distribution
        and receipt.proposed_action == "veto"
    )
...
def _tournament_policy_value(
    ...
    if checkpoint is None or checkpoint.net_return is None:
        return None
    if receipt.proposed_action not in {"support", "veto"}:
        return None
    return 0.0 if _veto_applies(receipt) else checkpoint.net_return
...
def _entry_validation_current(value: LearningModel | ChallengerSkillArtifact) -> bool:
    if isinstance(value, ChallengerSkillArtifact) and (
        value.skill != ChallengerSkill.ENTRY or value.schema_version == "challenger-skill-coach-v1"
    ):
        return True
    return value.hyperparameters.get("entry_validation_version") == ENTRY_VALIDATION_VERSION
...
def _top_mean(predictions: list[float], outcomes: list[float]) -> float:
    if (
        ...
        or not all(math.isfinite(value) for value in [*predictions, *outcomes])
    ):
        raise ValueError("invalid top-group validation population")
    count = max(1, len(outcomes) // 3)
    boundary = sorted(predictions, reverse=True)[count - 1]
    selected = [
        value for score, value in zip(predictions, outcomes, strict=True) if score > boundary
    ]
    tied = [value for score, value in zip(predictions, outcomes, strict=True) if score == boundary]
    # Fractional inclusion at the boundary treats equal forecasts equally. Outcomes never
    # decide membership, and an unchanged prediction population is independent of row order.
    return (math.fsum(selected) + (count - len(selected)) * fmean(tied)) / count
...
def _entry_family_proof_gates(
    ...
    *,
    ...
    """Explain saved family evidence under the current validation contract."""
    if artifact is None:
        return []
    metrics = dict(artifact.metrics)
    metrics["evidence_schema_current"] = bool(
        metrics.get("evidence_schema_current")
        ...
        gates.extend(
            [
                ...
                gate(
                    ...
                    "count",
                    "Enough Policy cases must be within learned support; "
                    "these may be kept or vetoed.",
                ),
                gate(
                    "policy_vetoes",
                    "Tested vetoes",
