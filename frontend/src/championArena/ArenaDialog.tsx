...
export default function ArenaDialog({ snapshot, selection, onClose, onSelect }: { snapshot: Snapshot; selection: Selection; onClose: () => void; onSelect: (selection: Selection) => void }) {
  ...
  const status = historical ? view.replayFinal === false ? "Recorded checkpoint" : "Saved result" : view.superseded ? "Saved profile" : view.mode === "interrupted" ? "Comparison changed" : view.paused ? snapshot.learning.mode === "off" ? "Learning paused" : "Proof paused" : view.mode === "battle" ? "Live comparison" : "Fighter profile";
